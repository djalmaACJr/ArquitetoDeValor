-- ============================================================
-- Seed de novo usuário — lançamentos de exemplo
--
-- Estende fn_sincronizar_usuario (trigger AFTER INSERT em auth.users) para,
-- além das contas e categorias, criar TRANSAÇÕES e uma TRANSFERÊNCIA de
-- exemplo — o usuário novo abre o app com dashboard/extrato povoados em vez
-- de telas vazias.
--
-- Decisões:
--   • Datas RELATIVAS a CURRENT_DATE (mês atual e anterior) — o exemplo
--     nunca "envelhece"; não há data fixa.
--   • Descrições terminam em "(exemplo)" para o usuário identificar e
--     excluir quando quiser.
--   • Transferência segue a invariante do par: 2 linhas com o mesmo
--     id_par_transferencia, débito prefixado "[Transf. saída]" e crédito
--     "[Transf. entrada]" (mesmas convenções da Edge Function
--     transferencias), categoria protegida "Transferências".
--   • Lançamentos com data futura no mês corrente entram como PENDENTE;
--     os demais, PAGO.
--   • O trigger fn_validar_isolamento_usuario valida contra NEW.user_id,
--     e contas/categorias são criadas antes na MESMA transação → os
--     INSERTs passam na validação.
--
-- Idempotente: CREATE OR REPLACE (o trigger em auth.users já existe).
-- ============================================================
