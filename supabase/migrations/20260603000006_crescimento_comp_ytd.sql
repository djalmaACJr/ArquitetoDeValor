-- ============================================================
-- CRESCIMENTO: corrige o cálculo do ano corrente (v_comp_sum)
-- para somar apenas até CURRENT_DATE, tornando-o proporcional
-- ao período do ano anterior (que já usa v_cutoff_prev = YTD).
--
-- Antes, o ano corrente somava o ano INTEIRO, incluindo
-- projeções futuras (PROJECAO/PENDENTE), e era comparado contra
-- o YTD do ano anterior — produzindo um % desproporcional
-- (ex.: -129,97% quando o YoY YTD real era -70,1%).
--
-- Recria as DUAS funções (trigger + callable) idempotentemente,
-- adicionando "AND t.data <= CURRENT_DATE" ao v_comp_sum.
-- ============================================================
...
BEGIN
    ...
    -- ── CRESCIMENTO: líquido (receitas − despesas) por categoria ─
    ELSIF NEW.tipo = 'CRESCIMENTO' THEN
        ...
        IF array_length(v_cats, 1) > 0 THEN
            ...
            IF v_ano_comp > v_ano_base THEN
                -- Cutoff YTD para comparação justa no ano parcial
                ...
                -- Líquido do ano anterior (receitas − despesas)
                SELECT COALESCE(SUM(
                ...
                -- Líquido do ano corrente (receitas − despesas) — somente até hoje,
                -- para ficar proporcional ao YTD do ano anterior (exclui projeções).
                SELECT COALESCE(SUM(
                    CASE WHEN t.tipo='RECEITA' THEN  t.valor
                         WHEN t.tipo='DESPESA' THEN -t.valor
    ...
    ELSIF v.tipo = 'CRESCIMENTO' THEN
        ...
        IF array_length(v_cats, 1) > 0 THEN
            ...
            IF v_ano_comp > v_ano_base THEN
                ...
                -- Ano corrente: somente até hoje (proporcional ao YTD anterior)
                SELECT COALESCE(SUM(
                    CASE WHEN t.tipo='RECEITA' THEN  t.valor
                         WHEN t.tipo='DESPESA' THEN -t.valor
                         ELSE 0 END), 0) INTO v_comp_sum
