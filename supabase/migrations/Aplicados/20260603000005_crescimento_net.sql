-- ============================================================
-- CRESCIMENTO: usa valor LÍQUIDO (receitas − despesas)
-- por categoria, em vez de somar apenas créditos.
-- Necessário para categorias de investimento onde existem
-- tanto entradas (ganho) quanto saídas (taxa, imposto).
-- ============================================================
    ...
    ELSIF NEW.tipo = 'CRESCIMENTO' THEN
        ...
        IF array_length(v_cats, 1) > 0 THEN
            ...
            IF v_ano_comp > v_ano_base THEN
                ...
                -- Líquido do ano corrente (receitas − despesas)
                SELECT COALESCE(SUM(
                    CASE WHEN t.tipo='RECEITA' THEN  t.valor
                         WHEN t.tipo='DESPESA' THEN -t.valor
