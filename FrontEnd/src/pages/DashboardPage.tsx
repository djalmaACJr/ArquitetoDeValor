  ...
  return (
    <div className="bg-[#1a1f2e] rounded-xl p-4 relative overflow-hidden h-full">
      ...
      <div className="relative flex items-center justify-between mb-2">
        ...
        {!isMesAtual && (
          <span className="text-[15px] bg-blue-400/10 border border-blue-400/30 rounded-md text-av-blue px-2 py-1">
            {mes.split('-')[1]}/{mes.split('-')[0]}
          </span>
        )}
      </div>
      <p className="relative text-[32px] font-bold text-av-green leading-none">
        {oculto ? OCULTO : formatBRL(saldoExibido)}
      </p>
      <p className="relative text-[15px] text-white/35 mt-1.5">
        {oculto ? '-' : labelData}
      </p>
    ...
    <div className="border-b border-gray-100 dark:border-gray-700 last:border-0">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[16px] font-semibold text-gray-700 dark:text-gray-200 text-left">
            {nomeConta}
          </span>
        ...
        <span className="text-[16px] font-bold whitespace-nowrap" style={{ color: corTotal }}>
    ...
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
        ...
        >
          ...
          <span className="text-[16px] font-semibold text-gray-700 dark:text-gray-200">{titulo}</span>
          <span className="text-[16px] font-bold" style={{ color: cor }}>{formatBRL(Math.abs(filtravel ? totalFiltrado : total))}</span>
      ...
      {aberto && <div className="max-h-[240px] overflow-y-auto">
        {Object.keys(porConta).length === 0 && (
          <p className="text-[16px] text-gray-400 text-center py-6">Nenhum lançamento pendente</p>
        )}
      ...
      {aberto && (
        <button
          onClick={onVerTodos}
          className="w-full text-[15px] text-av-blue py-2 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors border-t border-gray-100 dark:border-gray-700"
        >
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700">
              <div>
                <p className="text-[18px] font-bold text-gray-800 dark:text-gray-100">{titulo} — todas as categorias</p>
            ...
            <div className="overflow-y-auto px-5 pb-5">
              {dados.length === 0 ? (
                <p className="text-[16px] text-center text-gray-400 py-8">Nenhuma categoria neste período.</p>
              ) : dadosFiltrados.length === 0 ? (
                <p className="text-[16px] text-center text-gray-400 py-8">
                ...
                <table className="w-full border-collapse">
                  <thead>
                    ...
                    <tr className="text-[14px] font-bold uppercase tracking-wider text-gray-400">
                    ...
                    {dadosFiltrados.map((d) => {
                      ...
                      return (
                        <tr key={d.categoria_id} className="border-b border-gray-100 dark:border-gray-700/50 last:border-0">
                          <td className="py-2 text-[15px] text-gray-400">{idxOriginal + 1}</td>
                          <td className="py-2 text-[16px] text-gray-700 dark:text-gray-200">
                            <div className="flex items-center gap-2">
                              <span className="w-2 h-2 rounded-sm flex-shrink-0" style={{ background: cor }} />
                              <span>{d.categoria_nome}</span>
                            </div>
                          </td>
                          <td className="py-2 text-right text-[16px] font-semibold text-gray-700 dark:text-gray-200">{formatBRL(d.total)}</td>
                          <td className="py-2 text-right text-[15px] text-gray-500">{pct.toFixed(1)}%</td>
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-gray-200 dark:border-gray-600">
                      <td colSpan={2} className="py-2.5 text-[15px] font-bold uppercase tracking-wider text-gray-600 dark:text-gray-300">
                        {busca.trim() ? 'Subtotal' : 'Total'}
                      </td>
                      <td className="py-2.5 text-right text-[17px] font-bold text-gray-800 dark:text-gray-100">{formatBRL(totalFiltrado)}</td>
                      <td className="py-2.5 text-right text-[15px] text-gray-500">
    ...
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[16px] font-semibold text-gray-500 dark:text-gray-400">Últimas alterações</span>
        </div>
      ...
      >
        ...
        <div className="flex items-center justify-between px-3 py-2.5 border-b border-white/8 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[17px] font-semibold" style={{ color: '#e8eaf0' }}>Lembretes</span>
          </div>
        ...
        <div className="overflow-y-auto flex-1 p-2 space-y-3">
          {loading ? (
            <div className="flex items-center justify-center py-10">
              <span className="text-[16px]" style={{ color: '#4a5168' }}>Carregando...</span>
            </div>
          ) : filtrados.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 gap-2">
              <Bell size={24} style={{ color: '#4a5168' }} />
              <span className="text-[16px]" style={{ color: '#4a5168' }}>
            ...
            Object.entries(porMes).map(([mes, lembs]) => (
              <div key={mes}>
                <div
                  className="text-[14px] font-semibold uppercase tracking-wider px-1 pb-1 mb-1"
                ...
                <div className="space-y-1">
                  {lembs.map(l => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className="text-[14px] font-mono flex-shrink-0 w-[32px]"
