...
import { TrendingUp, TrendingDown, Percent, Trophy, Calendar, Layers, Zap, ChevronDown, ChevronUp } from 'lucide-react'
...
function RankingCategorias({ categorias }: { categorias: InvestimentoRankingCategoria[] }) {
  ...
  const totalPct     = totalInicio > 0 ? ((totalMercado - totalInicio) / totalInicio) * 100 : 0
  return (
        ...
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {categorias.map((cat, idx) => (
              <tr key={cat.tipo_ativo} className="border-t border-white/5">
...
function QuadroCategoria({ categoria, ativos, periodo, origem, dragHandleProps, dropTargetProps, bordaClasse }: {
  ...
  return (
    <div data-quadro-arrastavel className={`rounded-xl bg-white/[0.02] p-4 transition-all duration-150 ${bordaClasse}`} {...dropTargetProps}>
      {/* div, não <button>: um <button> engolindo a alça arrastável dentro
          dele impede o `dragstart` de disparar no Firefox (bug conhecido,
          Mozilla #646823) — o arraste simplesmente nunca começava. */}
      <div role="button" tabIndex={0} onClick={() => setAberto(!aberto)}
...
export default function DestaquesInvestimentosPage() {
  ...
  const { destaques, loading, error } = useInvestimentosDestaques(contaId || null, periodo)
  ...
  return (
    <div className="p-5">
      ...
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <h1 className="text-[22px] font-bold text-white">Destaques da carteira</h1>
      ...
      {loading ? <LoadingMascote /> : ativos.length === 0 ? (
        ...
        <>
          <RankingCategorias categorias={categorias} />
