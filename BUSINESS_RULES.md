...
### P/VP e dados da CVM (FII)

**P/VP** (preço ÷ Valor Patrimonial por cota) é sempre **calculado no frontend** (`precoAtual(l) / l.meta.fii_vp`, `QuadroTipoAtivos.tsx`/`DetalheInvestimentoPage.tsx`) — nunca persistido; o que fica salvo é só o VP (`inv_ativos.fii_vp`).

Nem brapi nem Yahoo trazem fundamentos de FII brasileiro (book value por cota não existe como campo pra esse tipo de ativo nesses provedores) — o **único dado público e oficial** disponível é o Informe Mensal da CVM. Implementado em `cvm.ts`, com **DUAS fontes** (o app não distingue FIAGRO de FII no schema — `tipo_ativo` só tem `FII` — mas a CVM trata como categorias separadas, com datasets/formato/escala diferentes):

- **FII** (`dados.cvm.gov.br/dados/FII/DOC/INF_MENSAL/DADOS/inf_mensal_fii_<ANO>.zip`) — ZIP **anual**, acumula 1 linha por fundo a cada mês que reportar durante o ano, em 2 CSVs (`;`, **Latin-1**, não UTF-8): `inf_mensal_fii_geral_<ANO>.csv` (`CNPJ_Fundo_Classe`, `Codigo_ISIN`, `Segmento_Atuacao`, `Mandato`) e `inf_mensal_fii_complemento_<ANO>.csv` (`Valor_Patrimonial_Cotas`, `Total_Numero_Cotistas`, `Percentual_Dividend_Yield_Mes` — **em fração**, ex. `0.0084` = 0,84%, convertido ×100 na gravação).
- **FIAGRO** (`dados.cvm.gov.br/dados/FIAGRO/DOC/INF_MENSAL/DADOS/inf_mensal_fiagro_<ANOMES>.zip`) — ZIP **mensal** (não anual), 1 CSV combinado por mês (geral+complemento juntos): `CNPJ_Classe` (⚠️ nome de campo diferente do FII), `Codigo_ISIN`, `Classificacao_Autorregulada` (mapeado pra `fii_mandato`; sem equivalente a `Segmento_Atuacao` → `fii_segmento` fica `null`), `Valor_Patrimonial_Cotas`, `Numero_Cotistas`, `Dividend_Yield_Mes` (⚠️ **já em %**, ex. `1.18` = 1,18% — **sem** ×100, escala diferente do FII, confirmado contra dado real de SNAG11/Suno Agro). Fundos atrasam a entrega ~1 mês — busca os últimos 4 meses disponíveis (lista o diretório pra descobrir quais existem) e funde, pegando o mais recente por fundo.
- Por fundo, usa sempre a linha de **maior `Data_Referencia`** e, empatada, **maior `Versao`** (retificação do mesmo mês) — em ambas as fontes.
- **Ticker → CNPJ**: a CVM identifica fundos por CNPJ, não pelo código de negociação B3. A ponte é o `Codigo_ISIN` (formato `BR<4 letras>CTF<dígito>`) — as 4 letras centrais SÃO a raiz do ticker B3 (`BRMXRFCTF008` → `MXRF11`, `BRSNAGCTF000` → `SNAG11`, confirmado contra dado real). Convenção estável (B3/CVM coordenam a emissão do ISIN), sem tabela de mapeamento própria. FII tem prioridade sobre FIAGRO em caso de colisão de raiz (não deveria acontecer — tickers B3 são únicos).
- **Prioridade**: a CVM **sempre sobrescreve** um `fii_vp` digitado manualmente quando encontra o fundo (`fii_vp_origem` vira `'CVM'`) — só fica `'MANUAL'` permanentemente pra fundos fora da cobertura das duas fontes (muito novo, sem negociação, etc.).
- **Gatilhos**: cron semanal `cvm-fii-cron` (todos os usuários, todos os FIIs — ver ARCHITECTURE.md § Cron jobs) + busca síncrona best-effort no cadastro de um FII novo (`POST /investimentos/ativos`, pra não esperar até 7 dias pelo 1º valor). Edição de um FII já existente (PUT) **não** dispara busca síncrona — só marca `fii_vp_origem='MANUAL'` quando o usuário mexe no campo; a atualização de verdade vem do cron semanal seguinte.
- Cache em memória do processo da Edge Function (TTL 1h) evita rebaixar/parsear o ZIP a cada FII cadastrado em sequência.

### Preço Teto (Bazin) e Valor Justo (Graham) — Ações

Dois indicadores de valuation exclusivos de `tipo_ativo = 'ACOES'`, com fontes de dado (e por isso escopo de implementação) **completamente diferentes**:

- **Preço Teto** (método Bazin): dividendo pago nos últimos 12 meses ÷ quantidade atual ÷ **6% fixo** (yield mínimo exigido, padrão clássico de Décio Bazin). **Calculado no frontend**, sem persistência nem fonte nova — usa os mesmos `inv_dividendos` já carregados (`DetalheInvestimentoPage.tsx`) ou o `dividend_yield_pct` (DY 12m) já exposto pelo ranking (`QuadroTipoAtivos.tsx`, evitando buscar dividendos ativo a ativo no grid). `null` sem posição ou sem dividendo pago no período.
- **Valor Justo** (fórmula de Graham): `√(22,5 × LPA × VPA)`. **Calculado e persistido pelo backend** (`inv_ativos.acao_valor_justo`) a partir de `acao_lpa`/`acao_vpa`, únicos campos que precisam de uma fonte de fundamentos nova — implementado em `cvmAcoes.ts`, mesmo padrão de `cvm.ts` (FII), mas com datasets diferentes da CVM:
  - **FCA** (`dados.cvm.gov.br/dados/CIA_ABERTA/DOC/FCA/DADOS/fca_cia_aberta_<ANO>.zip`) — ZIP anual: `fca_cia_aberta_valor_mobiliario_<ANO>.csv` (`CNPJ_Companhia`, `Codigo_Negociacao`, `Mercado`) é a **ponte ticker → CNPJ** (papel equivalente ao Código ISIN do FII) — dedup pela linha mais recente por `Codigo_Negociacao`, filtrando `Mercado = 'Bolsa'`. `fca_cia_aberta_capital_social_<ANO>.csv` (`CNPJ_Companhia`, `Quantidade_Total_Acoes`) dá o nº total de ações usado como denominador de LPA e VPA.
  - **DFP** (`dados.cvm.gov.br/dados/CIA_ABERTA/DOC/DFP/DADOS/dfp_cia_aberta_<ANO>.zip`) — ZIP anual do **exercício anterior** (o corrente só fecha 31/12 e a empresa tem ~3 meses pra publicar): `dfp_cia_aberta_BPP_con_<ANO>.csv` (conta `2.03` = Patrimônio Líquido Consolidado) e `dfp_cia_aberta_DRE_con_<ANO>.csv` (conta `3.11` = Lucro/Prejuízo Consolidado do Período), filtrados por `ORDEM_EXERC = 'ÚLTIMO'`. Fallback pra `_ind_` (individual) quando a empresa não publica consolidado (sem subsidiária). `LPA = lucro ÷ nAções`, `VPA = patrimônioLíquido ÷ nAções`.
  - **Deliberadamente anual (DFP), não trimestral (ITR)**: LPA/VPA mudam no máximo 1x/ano por empresa — evita reconstruir período acumulado do ITR (mesma classe de risco de bug de escala já vista no DY do FIAGRO, ver seção acima).
  - `Valor Justo = null` quando LPA ou VPA `<= 0` (Graham não se aplica a empresa no prejuízo/patrimônio líquido negativo) — fórmula centralizada em `calcularValorJustoGraham()`, reusada pelo cron E pela edição manual de LPA/VPA em `ativos.ts`, pra nunca haver 2 jeitos de calcular a mesma coisa divergindo.
  - **Prioridade/gatilhos**: mesmo padrão do FII — CVM sempre sobrescreve LPA/VPA digitados manualmente quando encontra a empresa (`acao_fundamentos_origem` vira `'CVM'`); cron mensal `cvm-acoes-cron` (todos os usuários, todas as ações — ver ARCHITECTURE.md § Cron jobs) + busca síncrona best-effort no cadastro de uma ação nova; PUT não dispara busca síncrona, só marca `'MANUAL'`.
  - Cache em memória do processo (TTL 1h, mesmo motivo do FII).
...
**Títulos "com Juros Semestrais" (NTN-F/NTN-B tradicional) e o reset do juros-sobre-juros**: quando o PU de marcação a mercado está disponível (caso normal), ele já é o preço real de mercado — cai sozinho no dia do cupom, sem precisar de tratamento especial. Só no **fallback** (acumulação por índice, usado quando falta PU pro título/mês) é que isso importava: a série `valorRFAcumulado`/`valorRF` aceita um `datasReset` opcional (datas de cupom já pago, vindas do mesmo CSV do STN usado pro provento — `datasCupomParaAtivo` em `mercado.ts`) e reinicia a composição de juros no cupom mais recente dentro da janela — sem isso, o fallback continuava compondo sobre um valor que já tinha sido pago em dinheiro, inflando a posição (achado real, ago/2026).

**Provento do cupom semestral**: diferente de ações/FII (busca por ticker via B3), o cupom do Tesouro vem de um CSV público único do STN (Tesouro Transparente, dataset "Resgates do Tesouro Direto" → recurso "Pagamento de Cupom de Juros") casado contra TODOS os ativos `TESOURO_DIRETO` "com Juros Semestrais" de uma vez — sem chamada HTTP por ativo, sem rate limit de fonte externa. Só processa eventos com `data_resgate >= hoje` (nunca retroativo — cupons já pagos o usuário lança/lançou na mão; o CSV do STN tem defasagem de alguns meses, reconciliar pra trás arriscaria sobrescrever correção manual). `tipo_dividendo`: procura um tipo do usuário com "tesouro" no nome (`ILIKE`), priorizando o que tiver categoria mapeada — não há um tipo "Tesouro" seedado por padrão como os 4 de ações/FII (`trg_seed_tipos_dividendo`), o usuário cria o seu.
...
### Data COM de proventos (aviso de login + calendário)

"Data COM" é a última data em que o investidor precisa ter o ativo em carteira pra ter direito ao próximo provento anunciado (depois dela o papel negocia "ex"). Capturada junto com o resto do provento no cron `dividendos-cron-br` (fonte B3, mesmo endpoint já usado pra `paymentDate`/`rate`/`label` — ver `B3CashDividend`/`mapearCashDividends` em `dividendos.ts`) e persistida em `inv_dividendos.data_com` (nullable — proventos antigos/manuais não têm).

⚠️ **Nome de campo assumido, não verificado ao vivo**: `B3CashDividend.lastDatePrior` é o nome de campo mais provável pra essa data no JSON bruto da B3 (convenção usada por scrapers públicos do mesmo endpoint), mas não há como confirmar a partir de ambiente sem acesso a `sistemaswebb3-listados.b3.com.br`. Best-effort: se o nome real for outro, `data_com` fica sempre `null` silenciosamente — nunca quebra o provento em si (mesmo padrão de tolerância a fonte externa do resto do módulo).

Dois avisos de login, montados em `AppLayout.tsx` (mesmo canto/estilo de `NovidadesProventos`, empilhados):
- **Novidade de proventos** (já existente, `NovidadesProventos`/`useNovidadesProventos`): passou a exibir a Data COM junto com a data de pagamento quando a fonte a devolve — sem mecanismo novo, só um campo a mais no payload (`inv_dividendos_novidades`).
- **Data COM próxima** (novo, `AvisoDataComProxima`/`useAvisosDataCom`): avisa quando algum ativo com posição ativa tem Data COM nos próximos **5 dias corridos** (`DIAS_JANELA`), a partir de proventos ainda em `PROJECAO`. "Visto" é um **conjunto de chaves** `"ticker|data_com"` (`usuarios.datacom_avisos_vistos`, JSONB) — não um timestamp único como `cron_avisos_vistos_em`, porque o conjunto de ativos "na janela" muda todo dia (itens saem quando a data passa, outros entram); dispensar funde as chaves exibidas no momento e poda as que já passaram da data.

No calendário do Dashboard (`CalendarioDashboard`), a Data COM de proventos do mês exibido aparece como uma marcação própria (dot dourado `#eab308`), com ativo + valor projetado no painel de detalhe do dia — mesmo padrão de extensão de `diasNegativos`/`ultimasParcelas` (prop `Map<data, T[]>`, calculada em `DashboardPage.tsx` a partir de `useDividendos`).
...
- **FII também é segmentado por `fii_categoria`** (Tijolo/Papel/FoF/Desenvolvimento/FIAGRO/Outro — set/2026): um FII de Papel (CRI/CRA) e um de Tijolo (imóveis físicos) têm riscos opostos, então cada categoria tem suas próprias perguntas específicas (`PERGUNTAS_FII_POR_CATEGORIA` em `questionarioAtivos.ts`), além de 1 pergunta comum a todas (P/VP). `inv_questionarios` ganhou a coluna `fii_categoria` (`TEXT NOT NULL DEFAULT ''`, chave única `user_id, tipo_ativo, fii_categoria` — string vazia em vez de enum nullable de propósito, pra não quebrar `ON CONFLICT`: NULL nunca "colide" numa constraint UNIQUE). `''` é o custom **genérico** de FII (vale para qualquer categoria sem override); uma categoria concreta sobrepõe só os ativos daquela categoria. Resolução em `useInvQuestionarios.questionarioEfetivo(tipo, perfil, pesos, fiiCategoria)`: custom da categoria exata → custom genérico → padrão estático (que já monta o bloco certo pela categoria, caindo em `OUTRO` se a categoria do ativo não for reconhecida). Rotas `GET/PUT/DELETE /investimentos/questionarios/FII` e `POST .../gerar` aceitam `?categoria=` (ausente/vazio = genérico); o Mentor (IA) recebe a categoria no prompt e é instruído a focar nos riscos daquela categoria específica.
...
- **Busca na web nativa (opt-in por mentor, ago/2026)**: cada config de IA (`usuarios.ia_configs`) tem um campo `busca_web` (boolean) — só tem efeito em `claude`/`gpt`/`gemini` (`PROVEDORES_BUSCA_WEB` em `_shared/ia.ts`; os demais ignoram). Quando ligado, `chamarProvedorIA` liga a busca nativa do provedor (`web_search_20250305` da Anthropic, `google_search` do Gemini, `web_search_options` da OpenAI — este último só tem efeito em modelos "-search-preview"). Desligado por padrão (aumenta custo/latência); afeta a avaliação de ativos por mentores, o Conselho (`chat-mentor`, abaixo) e — desde que `lerConfigIAAtiva` passou a devolver `buscaWeb` — também o `chat_mascote`.

### Conselho — chat livre com todos os mentores sobre um ativo (`chat-mentor`, ago/2026)

- Página dedicada (`/investimentos/conselho`, item "Conselho" no `InvestimentosNav`). Complementa o `ChatMascote`: uma conversa 1:1 com um único mentor já é coberta pelo ícone do mascote em qualquer tela (as 4 personas pararam de recusar ativo nomeado — ver item abaixo), então esta página cobre só o modo "Conselho" — perguntar o mesmo a TODOS os mentores configurados de uma vez.
- `POST /investimentos/chat-mentor` (sem sub-rota — só existe esse modo): `lerMentoresIA` resolve todos os mentores configurados; body opcional `mentores_ids` (array de config id) restringe quais participam — vazio/ausente = todos (seletor de checkboxes na página, "Todos os mentores" como padrão). Os participantes respondem em paralelo com uma persona própria (`PERSONA_MENTOR_ATIVOS`, em `chatMentor.ts`, sem a restrição de ativo nomeado), sem exigir que o ativo esteja cadastrado em `inv_ativos`; a config ATIVA do usuário faz mais 1 chamada (independente do recorte de participantes), com persona de "presidente do conselho", sintetizando convergências/divergências num texto de consenso. Falha de 1+ mentores não derruba a chamada (só erro 502 se nenhum responder; 400 se os `mentores_ids` informados não baterem com nenhuma config existente); falha só na síntese ainda devolve as respostas individuais.
- **Anexar carteira (ou parte dela) e/ou extrato**: dois anexos independentes, cada um com seu próprio toggle "enviar só 1x" (desliga sozinho após o envio, igual à política de anexo de tela do `ChatMascote`) — podem ir juntos na mesma mensagem (blocos concatenados antes de virar `contexto`):
  - **Carteira**: snapshot local (ticker/nome/tipo/quantidade/valor aplicado) a partir de `useInvestimentosPosicoes({status:'ATIVA'})`, agrupado por ativo — sem depender de cotação/valor de mercado. Um seletor de tipos (igual ao `SeletorEscopo` do `ChatMascote`) restringe a apenas alguns tipos antes de anexar.
  - **Extrato**: últimos 12 meses de operações (`useInvestimentosOperacoes({de: ...})`) + proventos (`useDividendos`) mesclados e ordenados por data, capados nos 60 registros mais recentes (`MAX_ITENS_EXTRATO`) pra não estourar o payload/tokens — mesma junção conceitual da página `ExtratoInvestimentosPage`, mas sem replicar o merge completo de rótulos/cores da UI.
  - Cada bloco vira texto via `serializarContexto` (mesmo helper de `ContextoIAContext.tsx`) e é enviado como `contexto` no body — o backend concatena com o de Fatos Relevantes antes da pergunta.
- Contexto factual: se a mensagem contiver um ticker com cara de FII (regex `[A-Za-z]{4}11`), busca Fatos Relevantes via `buscarFatosRelevantesParaAtivo` (mesma função da avaliação) — best-effort, nunca bloqueia a resposta.
- **Sem persistência** — histórico fica só no cliente (`useChatMentorAtivos`) e é reenviado a cada mensagem, mesmo padrão do `chat_mascote` em modo `configId`. Não há tabela nova, então não entra nas rotinas de backup/restore.

### Relaxamento das personas do ChatMascote para ativos nomeados (ago/2026)

As 4 personas em `chat_mascote/index.ts` proibiam recomendação de ativo nomeado (`LIMITES`). Ficou trocado por "PODE opinar sobre um ativo específico, sob a ótica [do território daquele mascote]" — cada mascote mantém sua lente (Conselheiro = longo prazo/disciplina, Arquiteta = encaixe no orçamento, Mago Gato = juros compostos/renda passiva, Raposa = risco×retorno/valuation) e pode sugerir o "Conselho" (`/investimentos/conselho`) pra uma análise cruzando várias IAs de uma vez. `lerConfigIAAtiva` (`_shared/ia.ts`) passou a devolver também `buscaWeb` (mesmo cálculo de `lerMentoresIA`), e `chat_mascote/index.ts` passa esse campo pra `chamarProvedorIA` — antes o chat do mascote nunca usava busca na web, mesmo com `busca_web=true` configurado.
